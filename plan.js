...
const weakest = bands => MODULES.filter(m => m in bands).sort((a, b) => bands[a] - bands[b])[0] || MODULES[0];
...
export function plan(today, tests, attempts) {
  ...
  const byDay = [
    ...
    drill[w],
    drill.listening,
    () => week % 2 ? S("speaking", "exam", 14, { label: "Speaking test" }) : S("writing", "drill", 20, { task: 1, label: "Writing drill · Task 1" }),
